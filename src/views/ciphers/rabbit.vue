    ...
    methods: {
        encryptRabbit(){
            if (this.textString !== '' && this.passStr !== ''){
                this.encString = this.CryptoJS.Rabbit.encrypt(this.textString, this.passStr);
        ...
        decryptRabbit(){
            if (this.textStringDecrypt !== '' && this.passStrDecrypt !== ''){
                let encData = this.CryptoJS.Rabbit.decrypt(this.textStringDecrypt, this.passStrDecrypt);
...
<template>
    <p class="text-2xl mb-3">Rabbit Encryption</p>
    <input
        v-model="passStr"
        type="text"
    ...
    <div class="flex gap-3">
        <button type="button" @click="encryptRabbit" class="inline-block mb-3 px-6 py-2.5 bg-blue-600 text-white font-medium text-xs leading-tight uppercase rounded shadow-md hover:bg-blue-700 hover:shadow-lg focus:bg-blue-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-blue-800 active:shadow-lg transition duration-150 ease-in-out">Encrypt</button>
    </div>
    <p v-show="showEncStr" class="mb-2 text-black dark:text-white text-lg">Rabbit encrypted string:</p>
    ...
    <p class="text-2xl mt-9 mb-3">Rabbit Decryption</p>
    <input
    ...
    ></textarea>
    <div class="flex gap-3">
        <button type="button" @click="decryptRabbit" class="inline-block mb-3 px-6 py-2.5 bg-blue-600 text-white font-medium text-xs leading-tight uppercase rounded shadow-md hover:bg-blue-700 hover:shadow-lg focus:bg-blue-700 focus:shadow-lg focus:outline-none focus:ring-0 active:bg-blue-800 active:shadow-lg transition duration-150 ease-in-out">Decrypt</button>
    </div>
    <p v-show="showDecStr" class="mb-2 text-black dark:text-white text-lg">Rabbit decrypted string:</p>
