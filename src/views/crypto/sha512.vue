...
export default {
    ...
    methods: {
        generateHash(){
            if(this.hmacStr === ''){
                if (this.textString !== ''){
                    this.hashString = this.CryptoJS.SHA512(this.textString);
                    this.showHash = true;
                } else {
                    this.showHash = false;
                }
                ...
                if (this.textString !== ''){
                    this.hashString = this.CryptoJS.HmacSHA512(this.textString, this.hmacStr);
                    this.showHash = true;
                } else {
                    this.showHash = false;
                }
...
<template>
    <p class="text-2xl mb-3">SHA-512 hash generator</p>
    <input
        v-model="hmacStr"
        type="text"
    ...
    <p v-show="showHash" class="mb-2 text-black dark:text-white text-lg">SHA-512 hash:</p>
