...
export default {
    ...
    methods: {
        generateHash(){
            if (this.textString !== ''){
                this.hashString = this.CryptoJS.RIPEMD160(this.textString);
                this.showHash = true;
            } else {
                this.showHash = false;
            }
...
<template>
    <p class="text-2xl mb-3">RIPEMD-160 hash generator</p>
    <textarea
        v-model="textString"
    ...
    <p v-show="showHash" class="mb-2 text-black dark:text-white text-lg">RIPEMD-160 hash:</p>
